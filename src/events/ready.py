...
async def send_one_time_button_message(client: discord.Client):
    """Send a one-time button message to the dev guild."""
    ...
    guild_key = str(target_guild_id)
    if guild_key in state:
    ...
    try:
        ...
        await channel.send(
            'A one-time message with a button (click to open modal)',
            view=view
        )

        # Persist that we've sent this message so we don't send it again
        state[guild_key] = {'createdAt': datetime.utcnow().isoformat()}
        await save_state(state)
        logger.info(f"One-time button message sent and recorded for guild {target_guild_id}")
    except Exception as err:
        logger.error(
            f"Failed to send one-time message to guild {target_guild_id}",
            {'error': str(err)}
        )
